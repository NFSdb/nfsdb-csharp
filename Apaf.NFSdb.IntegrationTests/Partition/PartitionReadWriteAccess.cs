...
{
    public class PartitionReadWriteAccess
    {

        private const string FOLDER_PATH = "MultiPartitoin-Journal";
        private const int TOTAL_PARITIONS = 3;
        private const int TOTAL_COUNT = (int)10E3;
        ...
        public class LargeJournalGenerator : RecordGenerator<LargeJournalPerformance.LargeJournal>
        {
            readonly Lazy<string> _randomText = new Lazy<string>(ReadResourceText);

            private static string ReadResourceText()
            {
                using (Stream text = typeof(LargeJournalPerformance.LargeJournal).Assembly.GetManifestResourceStream(
            ...
            protected override void GenerateValues(LargeJournalPerformance.LargeJournal item, DateTime timestamp, int i)
            {
        ...
        private static IJournal<LargeJournalPerformance.LargeJournal> OpenJournal(EFileAccess access)
        {
            string directoryPath = Path.Combine(Utils.FindJournalsPath(), FOLDER_PATH);
            ...
            return new JournalBuilder()
                ...
                .WithAccess(access)
                .ToJournal<LargeJournalPerformance.LargeJournal>();
        }

        private IJournal<LargeJournalPerformance.LargeJournal> CreateJournal(EFileAccess access = EFileAccess.ReadWrite)
        {
        ...
        public void GenerateRows()
        {
            ...
            var generator = new LargeJournalPerformance.LargeJournalGenerator();
        ...
        public void Should_open_minimum_files()
        {
            ...
            using (var j = OpenJournal(EFileAccess.Read))
            {
                ...
                var first = readTx.Items.OrderBy(r => r.Timestamp).First();
                Assert.That(first.Num1, Is.EqualTo(0));
            }
        } 
    }
}